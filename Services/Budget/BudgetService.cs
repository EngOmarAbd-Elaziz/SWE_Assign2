...
/// <summary>
/// Core business service responsible for managing budget cycles,
/// expenses, debts, categories, and financial calculations.
/// Acts as the main logic layer of the Masroofy application.
/// </summary>
public sealed class BudgetService : IBudgetService
{
    ...
    /// <summary>Gets current calculation strategy.</summary>
    public ICalculationStrategy GetStrategy() => _strategy;

    /// <summary>Sets calculation strategy dynamically.</summary>
    public void SetStrategy(ICalculationStrategy strategy) => _strategy = strategy;

    /// <summary>Gets active budget cycle for current user.</summary>
    public BudgetCycle? GetCycle() => _cycles.GetActiveCycle(_user.Id);

    /// <summary>
    /// Creates a new budget cycle for the user.
    /// </summary>
    ...
    /// <summary>Gets expenses for a cycle.</summary>
    public List<Expense> GetExpenses(int cycleId)
        => _expenses.GetExpenses(cycleId);

    /// <summary>Adds a new expense and updates cycle balance.</summary>
    ...
    /// <summary>
    /// Applies rollover logic if a new day has started within the cycle period.
    /// </summary>
    public void ApplyRolloverIfNeeded(BudgetCycle cycle)
    {
        var today = DateTime.Today;

        if (today <= cycle.LastRolloverDate.Date ||
            today < cycle.StartDate.Date ||
            today > cycle.EndDate.Date)
        {
            return;
        }
    ...
    /// <summary>Calculates safe daily spending limit using strategy pattern.</summary>
    public decimal CalculateSafeDailyLimit(BudgetCycle cycle, DateTime? forDate = null)
    {
        ...
        var debtImpact = _debts.GetDebts(_user.Id)
            .Sum(d => d.Type == "Borrowing" ? d.Amount : -d.Amount);

        var tempCycle = new BudgetCycle
        {
            RemainingBalance = remainingBeforeDate + debtImpact
        };

        return _strategy.CalculateSafeLimit(tempCycle, remainingDays);
    }

    /// <summary>Checks if yesterday was overspent.</summary>
    public bool WasYesterdayOverspent(BudgetCycle cycle)
    {
        ...
        var limit = CalculateSafeDailyLimit(cycle, yesterday);
        var spent = GetTotalSpentOnDate(cycle.Id, yesterday);

        return spent > limit;
    }

    /// <summary>Checks if 80% of budget has been used.</summary>
    ...
    {
        var expenses = GetExpenses(cycle.Id).OrderBy(e => e.Date);
        ...
        {
            sb.AppendLine($"{e.Id},{e.CycleId},{e.Amount:0.00},\"{e.Category}\",{e.Date:O}");
        }
    ...
    // Helper methods omitted for brevity (already clean)
}
